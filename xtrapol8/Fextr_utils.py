...
def get_cctbx_version():
    """
    Function to get the cctbx version
    """
...
def phase_transfer(
    miller_array, phase_source
):  # taken from fobs_minus_fobs_map. Phases cannot be added to miller_array but have to be inserted with the data
    tmp = miller.array(
...
def neg_neflecions_binning(miller_array, prefix, log=sys.stdout):
    ...
    print("\n************Completeness of extrapolated structure factors************")
    print(
        "bin  resolution range     #reflections  #neg.reflections  completeness(%)  compl.pos.reflections(%)",
        file=log,
    )
    print(
        "bin  resolution range     #reflections  #neg.reflections  completeness(%)  compl.pos.reflections(%)"
    ...
    for i_bin in miller_array.binner().range_all():
        ...
        print(
            "{:s} {:^15d} {:>5d} ({:>5.2f}%) {:>15.2f} {:>15.2f}".format(
                legend,
                f_bin.size(),
                neg,
                neg_percent,
                comp_bin_data[i_bin] * 100,
    ...
    assert (
        neg_lst.shape
        == neg_percent_lst.shape
        == bin_res_cent_lst.shape
        == comp_lst.shape
        == comp_true_lst.shape
    )
    ...
    lines_labels_1 = [
        ax.get_legend_handles_labels() for ax in [fig.axes[0], fig.axes[2]]
    ]
...
def compute_f_sigf(miller_array, prefix, log=sys.stdout):
    ...
    if x_point1 == 0:  # lowest resolution bin, try second smallest value
        x_point1 = np.where(
            np.abs(f_sigf_lst - 0.8) == np.partition(np.abs(f_sigf_lst - 0.8), 1)[1]
        )[0][0]
    if 0 < x_point1 < f_sigf_lst.shape[0] - 1:
        if (
            f_sigf_lst[x_point1 - 1] > f_sigf_lst[x_point1]
            and f_sigf_lst[x_point1 + 1] > f_sigf_lst[x_point1]
        ):
            # local minimum, try second smallest value
            x_point1 = np.where(
                np.abs(f_sigf_lst - 0.8) == np.partition(np.abs(f_sigf_lst - 0.8), 1)[1]
    ...
    x_point1 = np.where(
        np.abs(f_sigf_lst - 1.2) == np.partition(np.abs(f_sigf_lst - 1.2), 0)[0]
    )[0][0]
    if x_point1 == 0:  # lowest resolution bin, try second smallest value
        x_point1 = np.where(
            np.abs(f_sigf_lst - 1.2) == np.partition(np.abs(f_sigf_lst - 1.2), 1)[1]
        )[0][0]
    if 0 < x_point1 < f_sigf_lst.shape[0] - 1:
        if (
            f_sigf_lst[x_point1 - 1] > f_sigf_lst[x_point1]
    ...
    for log_file in refine_log_lst:
        if os.path.isfile(log_file):
            ...
            if "refmac" in log_file:  # refinement performed in refmac
                r_work = float(
                    [line for line in log if "R factor" in line][-1].split()[-1]
                )
                r_free = float(
...
def get_Fextr_stats(
    occ, fextr_ms, Fextr_type, fdif_ms, FoFo_type, outdir, log=sys.stdout
):
    ...
    if fextr_ms.data().size() == fdif_ms.data().size():
        ...
        print(
            "bin  resolution range  #reflections <{0}> <sig({0})> <{1}> <sig({1})>".format(
                FoFo_type, Fextr_type
            )
        )
        for i_bin in fextr_ms.binner().range_all():
            ...
            print(
                "{:s} {:^10d} {:> 10.4f} {:> 10.4f} {:> 10.4f} {:> 10.4f}".format(
                    legend,
                    fextr_ms_bin.size(),
                    fdif_data_avg,
                    fdif_sigmas_avg,
        ...
        assert (
            bin_res_cent_lst.size()
            == fextr_data_lst.size()
            == fextr_sigmas_lst.size()
            == fdif_data_lst.size()
            == fdif_sigmas_lst.size()
        ), "list sizes to plot F and sigmas not equal"

    else:  # If reflections are rejected, then fextr_ms.size() != fdif_ms.size()
        fdif_ms.setup_binner(n_bins=20)
        fextr_ms.setup_binner(n_bins=20)
        print(
            "bin  resolution range_{0}  #reflections_{0}  <{0}> <sig({0})> | bin  resolution range_{1}  #reflections_{1} <{1}> <sig({1})>".format(
                FoFo_type, Fextr_type
            )
        )
        for i_bin in fdif_ms.binner().range_all():
            ...
            fextr_sigmas_lst.append(fextr_sigmas_avg)
            print(
                "{:s} {:^20d} {:> 10.4f} {:> 10.4f} | {:s} {:^20d} {:> 10.4f} {:> 10.4f}".format(
                    legend_fdif,
                    fdif_ms_bin.size(),
                    fdif_data_avg,
                    fdif_sigmas_avg,
