        ...
        for res_group in chain.residue_groups():
            for atom_group in res_group.atom_groups():
                for a in res_group.atoms():
                    ...
                    i = a.fetch_labels()
                    info.append(
                        (i.resname, i.resseq, i.chain_id, i.altloc, i.name, i.i_seq + 1)
                    )
        ...
        for res_group in chain.residue_groups():
            for atom_group in res_group.atom_groups():
                for a in res_group.atoms():
                    i = a.fetch_labels()
                    if [
                        i.resname.lstrip().rstrip(),
                        i.resseq.lstrip().rstrip(),
                        i.chain_id.lstrip().rstrip(),
                        i.altloc.lstrip().rstrip(),
                    ] in residlst_corr:
                        coord.append(a.xyz)
                        info.append(
                            (
...
def blob_detection(
    map_object, threshold, peak, coord, radius, info, out=sys.stdout, residlst_out=None
):
    # neg = []
    map_object.open_map()
...
def do_blob_search(map_object, threshold, peak, coord_pdb, radius, info):
    ...
    print(
        "Found  %i %s blobs %s peak threshold... Now integrating and allocating them."
        % bla
    )
    ...
    for i in range(1, num_features_peak + 1):
        ...
        if (
            np.abs(blob).max() < peak
            or blob.size < 2
            or any([np.array_equal(blob, y) for y in blobs])
        ):
            continue
        blobs.append(blob)
        sm = blob.sum()
        ...
        if threshold < 0.0:
            ...
            mx = blob.max()
            argmax = blob.argmax()
        index_max = np.transpose(np.nonzero(labeled_array == feature_to_integrate))[
            argmax
        ]
        ...
        if (
            coord_pdb is not None
        ):  # if no pdb is provided (should not happen within Xtrapol8)
            if (
                radius is not None and D.min() <= radius
            ) or radius is None:  # only select blobs which are close enough to atoms
...
def check_inputs(
    map_name, pdb, radius, threshold, peak, resid_list_in=None, log=sys.stdout
):
    """
...
def print_results(blobs, out=sys.stdout, residlst_out=None):
    ...
    for blob in blobs:
        ...
        for b in blob:
            ...
            out.write(
                "%4s %4s %4s %3s %4s  %4s %7.3f %7.3f %8.2f %6s\n" % tuple(b[:-1])
            )
...
def map_explorer(
    ...
    if resid_list_in == None:
        ...
        if idx[0].shape[0] >= 1:
            mask = blobs[
                idx, -1
            ]  # Keep the coordinates of the blob rather than storing the whole mask
        else:
        ...
        print(
            "Residue list is provided, Z-scoring will not be applied. All peaks around the specified residue(s) will be included in the analysis"
        )
        mask = blobs[:, -1]
        shape1 = mask.shape[0]
        mask = mask.reshape(
        ...
        for i in range(
            shape1
        ):  # can use shape 1 because all blobs are used in the mask
...
class Maps(object):
    ...
    def coord_transform_setup(self):
        ...
        self.transfo_matrix = np.array(
            [
                [self.a, self.b * np.cos(self.gamma), self.c * np.cos(self.beta)],
                [
                    0,
                    self.b * np.sin(self.gamma),
                    -self.c * self.cosa_star * np.sin(self.beta),
                ],
    ...
    def get_coord_from_map_indices(self, indices, verbose=False):
        ...
        if verbose:
            print(
                "Indices %3i %3i %3i converted to x,y,z: %8.2f %8.2f %8.2f"
...
class CCP4_Maps(Maps):
    ...
    def open_map(self):
        ...
        self.origin = np.array(
            self.map_object.data.as_double().origin(), dtype=np.float32
        )
        self.unit_cell = np.array(
            self.map_object.unit_cell_parameters, dtype=np.float32
        )
...
class XPLOR_Maps(Maps):
    ...
    def open_map(self):
        ...
        self.unit_cell = np.array(
            self.map_object.unit_cell.parameters(), dtype=np.float32
        )
        self.data = self.map_object.data.as_numpy_array()
        self.coord_transform_setup()
