...
class GradientButton(metallicbutton.MetallicButton):
    def __init__(
        ...
        bmp=None,
        size=wx.DefaultSize,
        style=metallicbutton.MB_STYLE_DEFAULT,
        handler_function=None,
        user_data=None,
        start_color=(218, 218, 218),
        gradient_percent=0,
        highlight_color=(160, 190, 210),
        label_size=11,
        caption_size=8,
        button_margin=5,
        ...
        if user_data is None:
            user_data = "None"
        metallicbutton.MetallicButton.__init__(
            self,
            parent=parent,
            label=label,
            label2=label2,
...
class TabLog(wx.Panel):
    ...
    def __init__(self, parent, options):
        ...
        for i in range(len(self.LOG_ENTRIES)):
            ...
            if i == 3:
                label2 = " "
            self.buttons.append(
                GradientButton(self, label=self.LOG_ENTRIES[i], label2=label2)
            )

        self.ButtonSizer = wx.BoxSizer(wx.VERTICAL)
        for button in self.buttons:
            self.ButtonSizer.Add(
                button, proportion=0, flag=wx.EXPAND | wx.ALL, border=10
            )
            self.ButtonSizer.AddSpacer(10)
        self.mainSizer = wx.BoxSizer(wx.HORIZONTAL)
        self.LogTextCtrl = wx.TextCtrl(
            self, wx.ID_ANY, size=(800, 600), style=wx.TE_MULTILINE | wx.TE_READONLY
        )
    ...
    def buildLogSteps(self):
        try:
            ...
            occs = np.linspace(
                self.options.occupancies.low_occ,
                self.options.occupancies.high_occ,
                self.options.occupancies.steps + 1,
                endpoint=True,
            )
            N_occ = np.size(occs)
            self.occ = occs[0]
    ...
    def updateLog(self, line):
        ...
        try:
            if self.LOG_STEPS[self.indexLOG] in line:
                ...
                if self.indexLOG < len(self.LOG_STEPS) - 1:
                    self.indexLOG += 1
    ...
    def CreateCoot(self):
        ...
        bmp = wx.Image(
            os.path.join(script_dir, "pngs/coot_logo_small.png"),
...
class TabMainImg(ScrolledPanel):
    ...
    def __init__(self, parent):
        ...
        self.mainSizer.Add(
            self.plotSizer, 1, wx.ALIGN_CENTER_HORIZONTAL | wx.ALIGN_CENTER_VERTICAL
        )
        self.SetSizer(self.mainSizer)
        ...
        for fextr in [
            ...
            self.mapping["alpha_occupancy_determination_%s.pickle" % fextr] = (
                self.plot_alpha_occupancy_determination
            )
            self.mapping["%s_refinement_R-factors_per_alpha.pickle" % fextr] = (
                self.plot_refinement_Rfactors_per_alpha
            )
    ...
    def addPlot(self, pickle_file):
        ...
        self.plotSizer.Add(
            canvas, 0, wx.ALIGN_CENTER_VERTICAL | wx.ALIGN_CENTER_HORIZONTAL
        )
    ...
    def plot_k_estimation(self, pickle_file):
        ...
        ax1.plot(
            bin_res_cent_lst[:], k_av_lst[:], marker=".", label="Average k", color="red"
        )
        ...
        ax2.legend(
            ...
            labels,
            loc="lower right",
            bbox_to_anchor=(0.75, -0.05, 0.45, 0.5),
            fontsize="small",
            framealpha=0.5,
        )
    ...
    def plot_Riso_CCiso(self, pickle_file):
        with open(pickle_file, "rb") as stats_file:
            bin_res_cent_lst, r_work_lst, cc_work_lst, r_work, cc_work = pickle.load(
                stats_file
            )
        ...
        ax2.legend(
            ...
            labels,
            loc="lower right",
            bbox_to_anchor=(0.81, -0.05, 0.45, 0.5),
            fontsize="small",
            framealpha=0.5,
        )
        ax1.set_title(
            "Riso and CCiso for high resolution reflections",
            fontsize="medium",
            fontweight="bold",
        )
        self.figure.subplots_adjust(hspace=0.35, left=0.09, right=0.82, top=0.95)
    ...
    def plot_q_estimation(self, pickle_file="q_estimation.pickle"):
        ...
        try:
            with open(pickle_file, "rb") as stats_file:
                bin_res_cent_lst, q_av_lst, q_max_lst, q_min_lst = pickle.load(
                    stats_file
                )
        except:
            with open(pickle_file, "rb") as stats_file:
        ...
        ax1.set_title(
            "Average q for high resolution reflections",
            fontsize="medium",
            fontweight="bold",
        )
        self.figure.subplots_adjust(hspace=0.35, left=0.09, right=0.82, top=0.95)
    ...
    def plot_Neg_Pos_reflections(self, suffix, pickle_file):
        ...
        ax0.set_title(
            "%s: Negative ESFAs" % (maptype), fontsize="medium", fontweight="bold"
        )
        ...
        self.figure.subplots_adjust(
            hspace=0.25, wspace=0.5, left=0.09, right=0.88, top=0.95
        )
        canvas = FigureCanvas(self, -1, self.figure)
        return canvas
    ...
    def plot_refinement_Rfactors_per_alpha(self, prefix, pickle_file):
        ...
        ax0.plot(
            occ_lst, r_free_lst, color="blue", marker="s", markersize=5, label="Rfree"
        )
        ...
        ax0.set_title(
            "R-factors after reciprocal space refinement with %s" % (maptype),
            fontsize="medium",
            fontweight="bold",
        )
        self.figure.subplots_adjust(hspace=0.35, left=0.09, right=0.82, top=0.95)
    ...
    def plot_alpha_occupancy_determination(self, suffix, pickle_file):
        ...
        axes[0, 0].plot(
            alphas, pos_features, "o", color="green", label="Positive features"
        )
        axes[0, 0].plot(
            alphas,
            neg_features,
        ...
        axes[0, 1].plot(
            occupancies, pos_features, "o", color="green", label="Positive features"
        )
        axes[0, 1].plot(
            occupancies,
            neg_features,
        ...
        if alphafound:
            ...
            axes[0, 0].vlines(
                alpha, ymin=0, ymax=ymax, color="magenta", linestyles="dashed"
            )
            axes[0, 1].vlines(
                occ, ymin=0, ymax=ymax, color="magenta", linestyles="dashed"
            )
            axes[1, 0].vlines(
                alpha_CC,
                ymin=0,
                ymax=np.max(pearsonCC),
                color="magenta",
                linestyles="dashed",
            )
            axes[1, 1].vlines(
    ...
    def plot_Fextrsigmas(self, prefix, pickle_file="Fextr_binstats.pickle"):
        ...
        ax1 = ax0.twinx()
        for a in indices:
            occ, _, _, bin_res_cent_lst, fextr_data_lst, fextr_sigmas_lst, _, _ = (
                alldata[a]
            )
        ...
        ax1.legend(
            loc="lower right",
            bbox_to_anchor=(1.17, -0.05, 0.45, 0.5),
            fontsize="x-small",
            framealpha=0.5,
        )
    ...
    def plot_FoFosigmas(self, pickle_file="Fextr_binstats.pickle"):
        ...
        with open(pickle_file, "rb") as stats_file:
            _, FoFo_type, _, bin_res_cent_lst, _, _, fdif_data_lst, fdif_sigmas_lst = (
                np.array(pickle.load(stats_file))
            )
        ...
        ax0.legend(
            ...
            labels_1,
            loc="lower right",
            bbox_to_anchor=(0.73, -0.05, 0.45, 0.5),
            fontsize="xx-small",
            framealpha=0.5,
        )
        ax0.set_title(
...
class TabOccResults(ScrolledPanel):
    ...
    def setupUI(self):
        try:
            self.occ_list = [
                str(choice) for choice in self.options.occupancies.list_occ
            ]

        except TypeError:
            self.occ_list = [
                "%.3f" % choice
                for choice in np.linspace(
                    self.options.occupancies.low_occ,
                    self.options.occupancies.high_occ,
                    self.options.occupancies.steps + 1,
                    endpoint=True,
        ...
        self.occNfextrSizer.Add(
            self.best_occ_Static, 0, wx.ALIGN_CENTER_VERTICAL, border=5
        )
        ...
        self.mainSizer.AddSpacer(30)
        self.mainSizer.Add(
            self.occNfextrSizer, 0, wx.ALIGN_CENTER_VERTICAL | wx.ALL, border=5
        )
        self.mainSizer.AddSpacer(30)
        self.mainSizer.Add(
            self.ImgSizer, 1, wx.ALIGN_CENTER_HORIZONTAL | wx.ALIGN_CENTER_VERTICAL
        )
        self.SetSizer(self.mainSizer)
        self.SetAutoLayout(1)
    ...
    def onSelection(self, evt):
        ...
        if self.finished:
            self.best_occ_Static.SetLabel(
                "best estimation @ %s" % self.best_occ[self.fextr]
            )  #    self.OccChoice.FindString(s)
            if float(self.occ) == float(self.best_occ[self.fextr]):
                try:
                    fn_ddm = re.sub(
                        ".png$", ".pickle", self.ddm[self.fextr]
                    )  # get name of pickle file for ddm. Not elegant
    ...
    def plot_FsigF(self, pickle_file):
        ...
        ax1.legend(
            loc="lower right",
            bbox_to_anchor=(0.79, -0.05, 0.45, 0.5),
            fontsize="xx-small",
            framealpha=0.5,
        )
        ax1.set_title(
    ...
    def plot_negativereflections(self, pickle_file):
        ...
        with open(pickle_file, "rb") as stats_file:
            bin_res_cent_lst, neg_lst, neg_percent_lst, comp_lst, comp_true_lst, s = (
                pickle.load(stats_file)
            )
        ...
        ax1.yaxis.label.set_color("red")
        ax1.set_title(
            "Negative ESFAs for high resolution reflections",
            fontsize="medium",
            fontweight="bold",
        )
        ax2 = ax1.twinx()
        ...
        lines_labels_1 = [
            ...
            for ax in [self.figure.axes[0], self.figure.axes[2]]
        ]
        lines_1, labels_1 = [sum(lne, []) for lne in zip(*lines_labels_1)]
        ax1.legend(
            lines_1,
            labels_1,
            loc="lower right",
            bbox_to_anchor=(0.82, -0.05, 0.45, 0.5),
            fontsize="xx-small",
            framealpha=0.5,
        )
        ...
        ax3.set_title(
            "Completeness for high resolution reflections",
            fontsize="medium",
            fontweight="bold",
        )
    ...
    def plot_ddm(self, pickle_file, scale=None):
        ...
        for chain in alldata:
            ...
            axs[row, col].set_title(
                "Chain %s" % ID, fontsize="medium", fontweight="bold"
            )
