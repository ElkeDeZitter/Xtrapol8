...
def run(args):
    ...
    if params.f_and_maps.fast_and_furious:
        Fextr_mtz_lst = []  # Still needs to be adapted with the dictionary
    ...
    for occ in params.occupancies.list_occ:
        ...
        for mp in final_maptypes:
            ...
            files_and_maps[mp]["2FextrFc"].append(
                os.path.abspath(Fextr.ccp4_name_2FoFc)
            )
            ...
            if (
                ...
                append_if_file_exist(
                    files_and_maps[mp]["recref_mtz_lst"], os.path.abspath(mtz_out)
                )
                append_if_file_exist(
                    files_and_maps[mp]["recref_pdb_lst"], os.path.abspath(pdb_rec)
                )
                append_if_file_exist(
                    files_and_maps[mp]["recrealref_lst"], os.path.abspath(pdb_rec_real)
                )
                append_if_file_exist(
                    files_and_maps[mp]["realref_lst"], os.path.abspath(pdb_real)
                )

            # fast_and_furious mode: no refinement, but need to keep track of structure factor files
            elif params.f_and_maps.fast_and_furious:
                append_if_file_exist(
                    Fextr_mtz_lst, os.path.abspath(Fextr.F_name)
                )  # still need to be updated with dictionary
    ...
    for mp in reversed_maptypes:
        ...
        # Estimate alpha first with difference_map maximization or PearsonCC.
        # alpha and occ will be overwritten later if "distance_analysis" is selected
        ...
        if (
            ...
            # Make a plot of the refinement R-factors, related to the specific maptype.
            # The log-files should have the same prefix as the mtz-files.
            plot_Rfactors_per_alpha(
                [
                    re.sub(r"mtz$", "log", fle)
                    for fle in files_and_maps[mp]["recref_mtz_lst"]
                ],
                mp_type,
            )
            print("", file=log)
            ...
            if len(files_and_maps[mp]["recrealref_lst"]) == len(
                set(files_and_maps[mp]["recrealref_lst"])
            ):
            ...
            if pdb_list == files_and_maps[mp]["recrealref_lst"]:
                ...
                if mp == "qFgenick_map":
                    ...
                    def _mqgenisck_to_ccp4(fle):
                        m = re.search(
                            r"(.+?)2mqFgenick-DFc(?:_phenix|_refmac)?_reciprocal", fle
                        )
                        if m:
                    ...
                    def _mkgenisck_to_ccp4(fle):
                        m = re.search(
                            r"(.+?)2mkFgenick-DFc(?:_phenix|_refmac)?_reciprocal", fle
                        )
                        if m:
                    ...
                    def _mgenisck_to_ccp4(fle):
                        m = re.search(
                            r"(.+?)2mFgenick-DFc(?:_phenix|_refmac)?_reciprocal", fle
                        )
                        if m:
            ...
            # Coot script
            mtzs_for_coot = []
            if len(files_and_maps[mp]["recref_mtz_lst"]) != len(
                params.occupancies.list_occ
            ):
                occ_list_mtz = [
                    (re.search(r"occupancy\_(.+?)\/", mtz).group(1))
                    for mtz in files_and_maps[mp]["recref_mtz_lst"]
                ]
                try:
                    mtz_rec = files_and_maps[mp]["recref_mtz_lst"][
                        occ_list_mtz.index(occ)
                    ]
                except ValueError:
                    mtz_rec = ""
            else:
                mtz_rec = files_and_maps[mp]["recref_mtz_lst"][
                    params.occupancies.list_occ.index(occ)
                ]
            ...
            pdbs_for_coot = [
                DH.pdb_in,
                files_and_maps[mp]["recref_pdb_lst"][
                    params.occupancies.list_occ.index(occ) + 1
                ],
                files_and_maps[mp]["realref_lst"][
                    params.occupancies.list_occ.index(occ) + 1
                ],
                files_and_maps[mp]["recrealref_lst"][
                    params.occupancies.list_occ.index(occ) + 1
                ],
            ]
            script_coot = open_all_in_coot(
