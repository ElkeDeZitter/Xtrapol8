    ...
    def check_symm(self, *hkls):
        for reflections in hkls:
            ...
            if SG_ok == False:
                err += 1
                err_m = (
                    "!!!Space group incompatibility: %s vs %s!!! Do you want to continue? This might just be nonsense."
                    % (
                        str(self.current_space_group),
                        str(reflections.crystal_symmetry().space_group_info()),
                    )
                )
        return err_m, err
...
class DataHandler:
    ...
    def check_outdir(self):
        ...
        while os.path.exists(outdir):
            ...
            if (
                i == 1000
            ):  # to avoid endless loop, but this leads to a max of 1000 Xtrapol8 runs
                break
    ...
    def open_files(self):
        ...
        """
        self.check_outdir()
        self.reflections_off = any_file(
            self.mtz_off, force_type="hkl", raise_sorry_if_errors=True
        )
        self.reflections_on = any_file(
            self.mtz_on, force_type="hkl", raise_sorry_if_errors=True
        )
        self.from_cif_create_pdb_file()
        self.model_in = any_file(
            self.check_and_delete_hydrogen(),
            force_type="pdb",
    ...
    def from_cif_create_pdb_file(self):
        ...
        if file_format == ".cif":
            ...
            p.close()
            if self.check_single_file(
                self.outdir + "/" + get_name(self.pdb_in) + ".pdb"
            ):
    ...
    def check_and_delete_hydrogen(self):
        ...
        if pdb_hier.hierarchy.remove_hd() != 0:
            print(
                "Model contains hydrogen atoms. Create a new model without these atoms in the output directory: %s"
                % (outname)
            )
            print(
                "Model contains hydrogen atoms. Create a new model without these atoms in the output directory: %s"
    ...
    def extract_fobs(self, low_res, high_res):
        ...
        """
        self.fobs_off, self.fobs_on = Column_extraction(
            self.reflections_off, self.reflections_on, low_res, high_res, log=log
        ).extract_columns()
    ...
    def check_additional_files(self):
        ...
        if self.additional != None:
            for fle in self.additional:
                if "cif" in fle:
                    try:
                        cif_object = mmtbx.monomer_library.server.read_cif(
                            file_name=fle
                        )
                    except Exception:
        ...
        crystal_symmetry = crystal.symmetry(
            unit_cell=self.UC, space_group_info=SpaceGroup
        )
    ...
    def generate_f_model(self, fobs, scattering_table="n_gaussian"):
        ...
        self.fmodel = mmtbx.f_model.manager(
            f_obs=fobs, r_free_flags=self.rfree, xray_structure=xray_structure
        )
    ...
    def update_fmodel(self, fobs):
        ...
        if self.fmodel.f_obs().data().size() > fobs.data().size():
            ...
            free = (
                self.fmodel.r_free_flags()
                .select(self.fmodel.r_free_flags().data() == True)
                .size()
            )
            frac = free / tot
            if frac < 0.048:
    ...
    def get_common_indices_and_Fobs_off(self, f_obs_on):
        ...
        if (
            check_common_indices(
                [f_model_scaled, f_obs_on, f_obs_off, f_obs_off_scaled, rfree]
            )
            == False
        ):
            f_model_scaled, f_obs_on = f_model_scaled.common_sets(f_obs_on)
            f_model_scaled, f_obs_off = f_model_scaled.common_sets(f_obs_off)
            f_model_scaled, f_obs_off_scaled = f_model_scaled.common_sets(
                f_obs_off_scaled
            )
            f_model_scaled, rfree = f_model_scaled.common_sets(rfree)
            if (
        ...
        print(
            "%d reflections rejected from off-state dataset, %d reflections left"
            % (off_ini - off_fin, off_fin)
        )
        print(
            "%d reflections rejected from on-state dataset, %d reflections left"
            % (on_ini - on_fin, on_fin)
        )
        print(
            "%d reflections rejected from off-state dataset, %d reflections left"
            % (off_ini - off_fin, off_fin),
    ...
    def check_common_indices(self, refl_lst):
        for a in refl_lst:
            for b in refl_lst:
                if b != a:
                    if (
                        a.indices().all_eq(b.indices()) == False
                        or a.data().size() != b.data().size()
                    ):
                        compatible = False
                        break
                    else:
    ...
    def scale_sigmas_from_fmodel(self):
        ...
        """
        sc = flex.sum(
            self.fobs_off_scaled.data() * self.f_model_scaled.amplitudes().data()
        ) / flex.sum(
            self.f_model_scaled.amplitudes().data()
            * self.f_model_scaled.amplitudes().data()
        )
        self.fobs_off_scaled = miller.array(
            miller_set=self.fobs_off,
...
class FobsFobs:
    ...
    def get_UC_and_SG(self):
        ...
        """
        self.SG = re.search(
            r"(.+?)\(No", self.fobs_off.space_group_info().symbol_and_number()
        ).group(1)
    ...
    def get_sigf(self):
        ...
        """
        self.sigf_diff = (self.fobs_on.sigmas() ** 2 + self.fobs_off.sigmas() ** 2) ** (
            0.5
        )
    ...
    def k_weighting(self, kweight_scale):
        ...
        """
        k_ms, self.k_av = calculate_k(
            self.fobs_off, self.fobs_on, kweight_scale=kweight_scale, log=log
        )
        self.k = k_ms.sigmas()
    ...
    def calculate_fdiff(self, kweight_scale=0.05):
        ...
        self.fdif_q_ms = make_miller_array(
            self.fdif_q, sigf_diff_q, self.SG, self.UC, self.indices
        )
        ...
        self.fdif_k_ms = make_miller_array(
            self.fdif_k, sigf_diff_k, self.SG, self.UC, self.indices
        )

        # calculate without weight
        self.outlier_rejection()
        self.fdif_c = self.c * self.fdif
        self.fdif_c_ms = make_miller_array(
            self.fdif_c, self.sigf_diff, self.SG, self.UC, self.indices
        )
...
class Fextrapolate:
    ...
    def __init__(
        ...
        if neg_refl_handle in [
            ...
            "truncate_no_fill",
            "absolute_no_fill",
        ]:  #'addconstant_no_fill', 'massage_no_fill'
    ...
    def fextr_q(self):
        ...
        self.fextr_ms = make_miller_array(
            fextr_q, self.fextr_sigmas_q(), self.SG, self.UC, self.indices
        )
    ...
    def fextr_k(self):
        ...
        self.fextr_ms = make_miller_array(
            fextr_k, self.fextr_sigmas_k(), self.SG, self.UC, self.indices
        )
    ...
    def fextr_noq(self):
        ...
        self.fextr_ms = make_miller_array(
            fextr, self.fextr_sigmas_noq(), self.SG, self.UC, self.indices
        )
    ...
    def fextr_calc_q(self):
        ...
        """
        fextr_q = (
            self.alf * self.fdif_q + self.fmodel_fobs_off.f_model().amplitudes().data()
        )
        self.fextr_calc_ms = make_miller_array(
            fextr_q, self.fextr_sigmas_q(), self.SG, self.UC, self.indices
        )
    ...
    def fextr_calc_k(self):
        ...
        """
        fextr_k = (
            self.alf * self.fdif_k + self.fmodel_fobs_off.f_model().amplitudes().data()
        )
        self.fextr_calc_ms = make_miller_array(
            fextr_k, self.fextr_sigmas_k(), self.SG, self.UC, self.indices
        )
    ...
    def fextr_calc_noq(self):
        ...
        """
        fextr = (
            self.alf * self.fdif + self.fmodel_fobs_off.f_model().amplitudes().data()
        )
        self.fextr_calc_ms = make_miller_array(
            fextr, self.fextr_sigmas_noq(), self.SG, self.UC, self.indices
        )
    ...
    def fextr_sigmas_q(self):
        ...
        return (
            (self.fobs_on.sigmas() * self.alf * q_factor) ** 2
            + (self.fobs_off.sigmas() * (1 - (q_factor * self.alf))) ** 2
        ) ** (0.5)
    ...
    def fextr_sigmas_k(self):
        ...
        return (
            (self.fobs_on.sigmas() * self.alf * k_factor) ** 2
            + (self.fobs_off.sigmas() * (1 - (k_factor * self.alf))) ** 2
        ) ** (0.5)
    ...
    def fextr_sigmas_noq(self):
        ...
        """
        return (
            (self.fobs_on.sigmas() * self.alf) ** 2
            + (self.fobs_off.sigmas() * (1 - self.alf)) ** 2
        ) ** (0.5)

    def message(self):
        print(
            "\n---CALCULATING %s TYPE OF ESFAS AND MAPS FOR OCCUPANCY %.3f (ALPHA = %.3f)---"
            % (self.maptype.upper(), self.occ, self.alf),
            file=log,
        )
        print(
            "\n---CALCULATING %s TYPE OF ESFAS AND MAPS FOR OCCUPANCY %.3f (ALPHA = %.3f)---"
    ...
    def negatives_zero(self, ms):
        ...
        return miller.array(miller_set=ms, data=data, sigmas=sigmas)
    ...
    def negatives_absolute(self, ms):
        ...
        data = ms.data().deep_copy()
        return miller.array(miller_set=ms, data=flex.abs(data), sigmas=ms.sigmas())
    ...
    def negatives_fcalc(self, ms):
        ...
        return miller.array(miller_set=ms, data=data, sigmas=ms.sigmas())
    ...
    def convert_to_I_then_to_F(
        self, ms, maptype, algorithm="reflection_file_converter"
    ):
        """
        ...
        Iextr = miller.array(miller_set=ms, data=I_data, sigmas=I_sigmas)
        ...
        if algorithm == "truncate":
            Corrected_Fs = Extrapolated_column_extraction(
                outname, labels, log
            ).get_Fs_from_truncate()
        else:
            Corrected_Fs = Extrapolated_column_extraction(
                outname, labels, log
            ).get_Fs_from_reflection_file_converter()
    ...
    def initial_maps(self, ms):
        ...
        FM = Filesandmaps(
            ...
            self.maptype,
            name_out,
            self.fmodel_fobs_off,
            crystal_gridding=self.crystal_gridding,
        )
        print(
            "Electron density map in which the negative calculated ESFAs are preserved, and without filling of missing reflections:"
        )
        print(
            "Electron density map in which the negative calculated ESFAs are preserved, and without filling of missing reflections:",
            file=log,
        ...
        try:
            mtz_name, ccp4_name_2FoFc, ccp4_name_FoFc = FM.write_Fextr_maps(
                fill_missing=False
            )
            print("  mtz-format: {:s}".format(mtz_name))
            print("  mtz-format: {:s}".format(mtz_name), file=log)
            print(
                "  ccp4-format: {:s} and {:s}".format(ccp4_name_2FoFc, ccp4_name_FoFc)
            )
            print(
                "  ccp4-format: {:s} and {:s}".format(ccp4_name_2FoFc, ccp4_name_FoFc),
                file=log,
            ...
            print(
                "  Cannot update and calculate scales for electron density maps. The reason might be the high number of negative ESFAs. The maps cannot be calculated."
            )
            print(
                "  Cannot update and calculate scales for electron density maps. The reason might be the high number of negative ESFAs. The maps cannot be calculated.",
                file=log,
    ...
    def negatives_and_maps_Fextr_Fextr_calc(self, ms, outdir_for_negstats=os.getcwd()):
        ...
        dump_negative_stats(
            ...
            self.maptype,
            ms.data().size(),
            neg_reflections.data().size(),
            outdir_for_negstats,
        )
        print(
            "Total number of negative ESFAs: %d (%.2f %% of the data)"
            % (
                neg_reflections.data().size(),
                neg_reflections.data().size() / ms.data().size() * 100,
            )
        )
        print(
            "Total number of negative ESFAs: %d (%.2f %% of the data)"
        ...
        if neg_reflections.data().size() > 0:
            ...
            if self.neg_refl_handle in ["reject_no_fill", "reject_and_fill"]:
                ...
                fextr_ms = self.convert_to_I_then_to_F(
                    ms, self.maptype, algorithm="truncate"
                )
                rfree, fmodel_fobs_off = self.get_updated_fmodel_fobs_off(fextr_ms)
                self.FM = Filesandmaps(
                    fextr_ms,
        ...
        try:
            ...
            if "no_fill" in self.neg_refl_handle:
                ...
                new_neg_refl_handle = "reject_and_fill"
            print(
                "  Cannot update and calculate scales for electron density maps. The reason might be the high number of negative ESFAs. The negative ESFAs will be removed and we try again. This means that %s will be used for this dataset instead of %s. This might impact further analysis and comparison of the electron density maps."
                % (new_neg_refl_handle, self.neg_refl_handle)
            )
            print(
                "  Cannot update and calculate scales for electron density maps. The reason might be the high number of negative ESFAs. The negative ESFAs will be removed and we try again. This means that %s will be used for this dataset instead of %s. This might impact further analysis and comparison of the electron density maps."
        ...
        print(
            "  electron density in ccp4-format: {:s} and {:s}".format(
                self.ccp4_name_2FoFc, self.ccp4_name_FoFc
            )
        )
        print(
            "  electron density in ccp4-format: {:s} and {:s}".format(
    ...
    def fextr(self, qweight=False, kweight=False, outdir_for_negstats=os.getcwd()):
        ...
        self.negatives_and_maps_Fextr_Fextr_calc(
            self.fextr_ms, outdir_for_negstats=outdir_for_negstats
        )
    ...
    def fgenick(self, qweight=False, kweight=False, outdir_for_negstats=os.getcwd()):
        ...
        print(
            ...
            % (
                neg_reflections.data().size(),
                neg_reflections.data().size() / self.fgenick_ms.data().size() * 100,
            )
        )
        print(
            "Total number of negative ESFAs: %d (%.2f %% of the data)"
    ...
    def fextr_calc(self, qweight=False, kweight=False, outdir_for_negstats=os.getcwd()):
        ...
        # Carry out handling of negative ESFAs and write files and maps.
        self.negatives_and_maps_Fextr_Fextr_calc(
            self.fextr_calc_ms, outdir_for_negstats=outdir_for_negstats
        )
    ...
    def get_updated_fmodel_fobs_off(self, miller_array):
        ...
        """
        fmodel_f_obs, miller_array = self.fmodel_fobs_off.f_obs().common_sets(
            miller_array
        )
    ...
    def refinements(
        ...
        if reciprocal_space_refinement == "phenix.refine":
            reciprocal = phenix_refinements.Phenix_reciprocal_space_refinement(
                mtz_F,
                pdb_in,
            ...
            density_modification = (
                phenix_keywords.density_modification.density_modification
            )
            ...
            reciprocal = ccp4_refmac.Refmac_refinement(
                mtz_F,
                pdb_in,
                additional,
                F_column_labels=F_column_labels,
            ...
            density_modification = (
                refmac_keywords.density_modification.density_modification
            )
        ...
        if density_modification and os.path.isfile(mtz_dm):
            ...
            pdb_out_rec_real = real.real_space_refinement_ccp4(
            ...
            pdb_out_rec_real = real.real_space_refinement_mtz(
                mtz_out_rec, pdb_out_rec, default_column_labels
            )
            print(
                "Output real space refinement after reciprocal space refinement:",
                file=log,
...
class Filesandmaps:
    ...
    def __init__(
        self, miller_array, rfree, maptype, prefix, fmodel_ref, crystal_gridding=None
    ):
        ...
        if self.maptype.lower() == "qfofo":
            ...
            self.labels = {
                "data": "QFEXTR",
                "map_coefs_map": "2QFEXTRFCWT",
                "map_coefs_diff": "QFEXTRFCWT",
            }
        elif self.maptype.lower() == "kfextr":
            self.labels = {
                "data": "KFEXTR",
                "map_coefs_map": "2KFEXTRFCWT",
                "map_coefs_diff": "KFEXTRFCWT",
            }
        elif self.maptype.lower() == "fextr":
            self.labels = {
                "data": "FEXTR",
                "map_coefs_map": "2FEXTRFCWT",
                "map_coefs_diff": "FEXTRFCWT",
            }
        elif self.maptype.lower() == "qfgenick":
            self.labels = {
                "data": "QFGENICK",
                "map_coefs_map": "QFGENICKWT",
                "map_coefs_diff": "QFGENICKFCWT",
            }
        elif self.maptype.lower() == "kfgenick":
            self.labels = {
                "data": "KFGENICK",
                "map_coefs_map": "KFGENICKWT",
                "map_coefs_diff": "KFGENICKFCWT",
            }
        elif self.maptype.lower() == "fgenick":
            self.labels = {
                "data": "FGENICK",
                "map_coefs_map": "FGENICKWT",
                "map_coefs_diff": "FGENICKFCWT",
            }
        elif self.maptype.lower() == "qfextr_calc":
            self.labels = {
                "data": "QFEXTR_CALC",
                "map_coefs_map": "2QFEXTR_CALCFCWT",
                "map_coefs_diff": "QFEXTR_CALCFCWT",
            }
        elif self.maptype.lower() == "kfextr_calc":
            self.labels = {
                "data": "KFEXTR_CALC",
                "map_coefs_map": "2KFEXTR_CALCFCWT",
                "map_coefs_diff": "KFEXTR_CALCFCWT",
            }
        elif self.maptype.lower() == "fextr_calc":
            self.labels = {
                "data": "FEXTR_CALC",
                "map_coefs_map": "2FEXTR_CALCFCWT",
                "map_coefs_diff": "FEXTR_CALCFCWT",
            }
        else:
            print(
                "map type %s not allowed. Using default dummy labels for the outpur files."
                % (self.maptype),
                file=log,
            )
            print(
            ...
            self.labels = {
                "data": "EXTR",
                "map_coefs_map": "2EXTRFCWT",
                "map_coefs_diff": "EXTRFCWT",
            }
    ...
    def mtz_file_Fs(self):
        ...
        mtz_dataset.add_miller_array(
            miller_array=self.rfree, column_root_label="FreeR_flag"
        )
    ...
    def mtz_mapcoefs(self, mc_map, mc_diff):
        ...
        """
        mtz_dataset = mc_map.as_mtz_dataset(
            column_root_label=self.labels["map_coefs_map"]
        )
        mtz_dataset.add_miller_array(
            miller_array=mc_diff, column_root_label=self.labels["map_coefs_diff"]
        )
    ...
    def write_FoFo_output(self):
        ...
        edm = map_tools_Millerset.electron_density_map(
            fobs_in=self.ms, fmodel_2=self.fmodel_ref
        )
        mc_mfofo = edm.map_coefficients(map_type="mfo")
        mtz_dataset = mc_mfofo.as_mtz_dataset(
            column_root_label=self.labels["map_coefs_diff"]
        )
        mtz_object = mtz_dataset.mtz_object()
    ...
    def write_Fextr_maps(self, fill_missing=True):
        ...
        """
        self.mtz_name = "%s_2m%s-DFc_m%s-DFc.mtz" % (
            self.prefix,
            self.maptype,
            self.maptype,
        )
    ...
    def write_Fextr_Fextr_calc_output(self, fill_missing=True):
        ...
        return self.F_name, self.mtz_name, self.ccp4_name_2FoFc, self.ccp4_name_FoFc
    ...
    def write_Fgenick_output(self):
        ...
        edm = map_tools_fomsource.electron_density_map(
            fmodel_1=fmodel_update, fmodel_2=self.fmodel_ref
        )
        mc_mfodfc = edm.map_coefficients(
            map_type="mfo-dfc", isotropize=True, fill_missing=False
        )
        mc_mfo = edm.map_coefficients(
            map_type="mfo", isotropize=True, fill_missing=False
        )
...
def run(args):
    ...
    qFextr_map = qFgenick_map = qFextr_calc_map = Fextr_map = Fgenick_map = (
        Fextr_calc_map
    ) = kFextr_map = kFgenick_map = kFextr_calc_map = False
    ...
    if params.f_and_maps.fofo_type == "fofo":
        ...
        remark = "%s not defined. Q-weighting will be applied." % (
            params.f_and_maps.fofo_type.__phil_path__()
        )
        remarks.append(remark)
        qFoFo_weight = True
        kFoFo_weight = False
    ...
    if params.f_and_maps.only_no_weight:
        qFextr_map = qFgenick_map = qFextr_calc_map = kFextr_map = kFgenick_map = (
            kFextr_calc_map
        ) = False
    ...
    if params.f_and_maps.only_qweight:
        Fextr_map = Fgenick_map = Fextr_calc_map = kFextr_map = kFgenick_map = (
            kFextr_calc_map
        ) = False
    ...
    if (
        params.refinement.run_refinement == False
        and params.map_explorer.occupancy_estimation == "distance_analysis"
    ):
    ...
    if (
        ...
        == kFextr_calc_map
        == False
        and params.f_and_maps.fast_and_furious == False
    ):
    ...
    if params.f_and_maps.fast_and_furious:
        ...
        params.f_and_maps.only_no_weight = params.f_and_maps.all_maps = (
            params.f_and_maps.only_kweight
        ) = params.f_and_maps.only_qweight = False
        ...
        kFoFo_weight = qFgenick_map = qFextr_calc_map = Fextr_map = Fgenick_map = (
            Fextr_calc_map
        ) = kFextr_map = kFgenick_map = kFextr_calc_map = False
        remarks.append(remark)
    ...
    occ_step = (
        params.occupancies.high_occ - params.occupancies.low_occ
    ) / params.occupancies.steps
    ...
    if len(occ_lst) == 0:
        remark = (
            "No input occupancies found. Xtrapol8 will stop after the FoFo calculation."
        )
        remarks.append(remark)
    ...
    print(
        "Extracting the unit cell and space group from input files. Check if these are similar for all input files:"
    )
    ...
    if err_off + err_on == 0:
        ...
        if err_off == 1:  # very weird if this would happen
            ...
            print(
                "Changing space group and/or unit cell parameters of the triggered data in order to match with the model as these might have been changed by Pointless.",
                file=log,
            )
            print(
                "Changing space group and/or unit cell parameters of the triggered data in order to match with the model as these might have been changed by Pointless."
    ...
    print(
        "----Summary reference scaled and sorted common reflections with triggered data:----"
    )
    ...
    print(
        "----Summary triggered scaled and sorted common reflections with reference data:----"
    )
    ...
    map_expl_out_FoFo, residlist_zscore, mask, FoFo_ref = map_explorer(
        FoFo.ccp4_name,
        DH.pdb_in,
        params.map_explorer.radius,
        params.map_explorer.peak_integration_floor,
    ...
    Map_explorer_analysis(
        peakintegration_file=map_expl_out_FoFo,
    ...
    if params.output.generate_fofo_only:
        ...
        print("-----------------------------------------", file=log)
        print(
            "DONE! FOURIER DIFFERENCE MAP CALCULATED AND ANALYSIS PERFORMED.", file=log
        )
        print("-----------------------------------------", file=log)
        ...
        if outname == "triggered":
            ...
            print(
                "Replacing the dummpy outname ('%s') with true outname('%s')"
                % (outname, params.output.outname)
            )
            tr = [
    ...
    for occ in params.occupancies.list_occ:
        ...
        for mp in final_maptypes:
            ...
            if mp == "qFextr_map":
                ...
                Fextr.fextr_calc(
                    qweight=True, kweight=False, outdir_for_negstats=outdir
                )
                get_Fextr_stats(
                    occ,
                    Fextr.fextr_calc_ms,
                ...
                Fextr.fextr_calc(
                    qweight=False, kweight=True, outdir_for_negstats=outdir
                )
                get_Fextr_stats(
                    occ,
                    Fextr.fextr_calc_ms,
                ...
                Fextr.fextr_calc(
                    qweight=False, kweight=False, outdir_for_negstats=outdir
                )
                get_Fextr_stats(
                    occ,
                    Fextr.fextr_calc_ms,
            ...
            print("\n************Map explorer************", file=log)
            print("\n************Map explorer************")
            data = ccp4_map.map_reader(
                file_name=Fextr.ccp4_name_FoFc
            ).data.as_numpy_array()
            pos = 0
            neg = 0
            ...
            try:
                ...
                print(
                    "Pearson correlation factor could not be calculated. The CC will be set to zero."
                )
                CC = 0
            ...
            if (
                ...
                and params.refinement.run_refinement
            ):
                print("\n************Refinements************")
                ...
                mtz_out, pdb_rec, pdb_real, pdb_rec_real = Fextr.refinements(
                    ...
                    phenix_map_column_labels="%s,PHI%s"
                    % (
                ...
                if mp == "qFextr_map":
                    append_if_file_exist(
                        qFextr_recref_mtz_lst, os.path.abspath(mtz_out)
                    )
                    append_if_file_exist(
                        qFextr_recref_pdb_lst, os.path.abspath(pdb_rec)
                    )
                    append_if_file_exist(
                        qFextr_recrealref_lst, os.path.abspath(pdb_rec_real)
                    )
                    append_if_file_exist(qFextr_realref_lst, os.path.abspath(pdb_real))
                elif mp == "qFgenick_map":
                    append_if_file_exist(
                        qFgenick_recref_mtz_lst, os.path.abspath(mtz_out)
                    )
                    append_if_file_exist(
                        qFgenick_recref_pdb_lst, os.path.abspath(pdb_rec)
                    )
                    append_if_file_exist(
                        qFgenick_recrealref_lst, os.path.abspath(pdb_rec_real)
                    )
                    append_if_file_exist(
                        qFgenick_realref_lst, os.path.abspath(pdb_real)
                    )
                elif mp == "qFextr_calc_map":
                    append_if_file_exist(
                        qFextr_calc_recref_mtz_lst, os.path.abspath(mtz_out)
                    )
                    append_if_file_exist(
                        qFextr_calc_recref_pdb_lst, os.path.abspath(pdb_rec)
                    )
                    append_if_file_exist(
                        qFextr_calc_recrealref_lst, os.path.abspath(pdb_rec_real)
                    )
                    append_if_file_exist(
                        qFextr_calc_realref_lst, os.path.abspath(pdb_real)
                    )
                elif mp == "kFextr_map":
                    append_if_file_exist(
                        kFextr_recref_mtz_lst, os.path.abspath(mtz_out)
                    )
                    append_if_file_exist(
                        kFextr_recref_pdb_lst, os.path.abspath(pdb_rec)
                    )
                    append_if_file_exist(
                        kFextr_recrealref_lst, os.path.abspath(pdb_rec_real)
                    )
                    append_if_file_exist(kFextr_realref_lst, os.path.abspath(pdb_real))
                elif mp == "kFgenick_map":
                    append_if_file_exist(
                        kFgenick_recref_mtz_lst, os.path.abspath(mtz_out)
                    )
                    append_if_file_exist(
                        kFgenick_recref_pdb_lst, os.path.abspath(pdb_rec)
                    )
                    append_if_file_exist(
                        kFgenick_recrealref_lst, os.path.abspath(pdb_rec_real)
                    )
                    append_if_file_exist(
                        kFgenick_realref_lst, os.path.abspath(pdb_real)
                    )
                elif mp == "kFextr_calc_map":
                    append_if_file_exist(
                        kFextr_calc_recref_mtz_lst, os.path.abspath(mtz_out)
                    )
                    append_if_file_exist(
                        kFextr_calc_recref_pdb_lst, os.path.abspath(pdb_rec)
                    )
                    append_if_file_exist(
                        kFextr_calc_recrealref_lst, os.path.abspath(pdb_rec_real)
                    )
                    append_if_file_exist(
                        kFextr_calc_realref_lst, os.path.abspath(pdb_real)
                    )
                elif mp == "Fextr_map":
                    ...
                    append_if_file_exist(
                        Fextr_recrealref_lst, os.path.abspath(pdb_rec_real)
                    )
                    append_if_file_exist(Fextr_realref_lst, os.path.abspath(pdb_real))
                elif mp == "Fgenick_map":
                    append_if_file_exist(
                        Fgenick_recref_mtz_lst, os.path.abspath(mtz_out)
                    )
                    append_if_file_exist(
                        Fgenick_recref_pdb_lst, os.path.abspath(pdb_rec)
                    )
                    append_if_file_exist(
                        Fgenick_recrealref_lst, os.path.abspath(pdb_rec_real)
                    )
                    append_if_file_exist(Fgenick_realref_lst, os.path.abspath(pdb_real))
                elif mp == "Fextr_calc_map":
                    append_if_file_exist(
                        Fextr_calc_recref_mtz_lst, os.path.abspath(mtz_out)
                    )
                    append_if_file_exist(
                        Fextr_calc_recref_pdb_lst, os.path.abspath(pdb_rec)
                    )
                    append_if_file_exist(
                        Fextr_calc_recrealref_lst, os.path.abspath(pdb_rec_real)
                    )
                    append_if_file_exist(
                        Fextr_calc_realref_lst, os.path.abspath(pdb_real)
                    )
    ...
    print(
        "Residue list used for estimation of occupancy of triggered state: %s\n"
        % (residlst),
        file=log,
    )
    print(
    ...
    for mp in reversed_maptypes:
        ...
        if params.map_explorer.occupancy_estimation in (
            "difference_map_maximization",
            "distance_analysis",
        ):
            ...
            alpha, occ, _, _ = plotalpha(
                params.occupancies.list_occ,
                map_expl_lst[1:],
                map_expl_lst[0],
                mp_type,
                log=log,
        ...
        if (
            ...
            if params.map_explorer.occupancy_estimation == "distance_analysis":
                alpha, occ = Distance_analysis(
                    pdb_list,
                    params.occupancies.list_occ,
                    resids_lst=residlst,
                    use_waters=distance_use_waters,
                    outsuffix=mp_type,
                    log=log,
                ).extract_alpha()
            else:
                _, _ = Distance_analysis(
                    pdb_list,
                    params.occupancies.list_occ,
                    resids_lst=residlst,
                    use_waters=distance_use_waters,
            ...
            if (
                len(ccp4_list)
                == len(pymol_pdb_list)
                == len(params.occupancies.list_occ)
            ):
                Pymol_movie(
                    params.occupancies.list_occ,
                    pdblst=pymol_pdb_list,
                    ccp4_maps=ccp4_list,
                    resids_lst=residlst,
                    model_label=model_label,
                    ccp4_map_label=ccp4_map_label,
                ).write_pymol_script()
            else:
                Pymol_movie(
                    params.occupancies.list_occ,
            ...
            ddm_out = Difference_distance_analysis(
                DH.pdb_in,
                pdb_for_ddm,
                ligands=DH.extract_ligand_codes(),
                outdir=occ_dir,
                scale=params.output.ddm_scale,
            ).ddms()
            print("---------------------------")
            ...
            if len(recref_mtz_lst) != len(params.occupancies.list_occ):
                occ_list_mtz = [
                    (re.search(r"occupancy\_(.+?)\/", mtz).group(1))
                    for mtz in recref_mtz_lst
                ]
                try:
            ...
            mtz_extr = [
                "%s/%s" % (occ_dir, fle)
                for fle in os.listdir(occ_dir)
                if outname in fle and fle.endswith("m%s-DFc.mtz" % (mp_type))
            ][0]
    ...
    if os.path.isfile("%s/pymol_movie.py" % (outdir)):
        Pymol_movie(
            params.occupancies.list_occ, resids_lst=residlst
        ).write_pymol_appearance("%s/pymol_movie.py" % (outdir))
    ...
    if params.f_and_maps.fast_and_furious and params.refinement.run_refinement:
        ...
        qFext_mtz_F = [
            fle
            for fle in Fextr_mtz_lst
            if "occupancy_%.3f" % (occ) in fle and fle.lower().endswith("_qfextr.mtz")
        ][0]
        assert os.path.isfile(qFext_mtz_F)
        print(
            "Extrapolated structure factors for reciprocal space refinement :%s"
            % (qFext_mtz_F),
            file=log,
        )
        print(
        ...
        qFext_mtz_map = re.sub(
            r"qFextr.mtz$", "2mqFextr-DFc_mqFextr-DFc.mtz", qFext_mtz_F
        )
        assert os.path.isfile(qFext_mtz_map)
        print(
            "Extrapolated map coefficients for real space refinement: %s"
            % (qFext_mtz_map),
            file=log,
        )
        print(
        ...
        pdbs_for_coot = [
            DH.pdb_in,
            os.path.abspath(check_file_existance(pdb_rec)),
    ...
    print("-----------------------------------------", file=log)
    print(
        "Please inspect the output and rerun with different parameters if necessary",
        file=log,
    )

    if params.f_and_maps.fast_and_furious == False and params.refinement.run_refinement:
        print(
            "Pymol script with models and maps: %s/pymol_movie.py." % (outdir), file=log
        )
