...
class Refiner:
    ...
    def search_mtz_file_reciprocal_space_refinement(self):
        ...
        if self.check_single_file(mtz_file):
            ...
            print(
                "File not for reciprocal space refinement not found: %s" % (mtz_file),
                file=log,
            )
            self.mtz_file = None
    ...
    def search_mtz_map_real_space_refinement(self):
        mtz_map = [
            ...
            if fle.startswith("%s_occ%.3f" % (self.outname, self.occ))
            and fle.endswith("%s-DFc.mtz" % (self.maptype))
        ][0]
        if self.check_single_file(mtz_map):
            ...
            print(
                "File not for real space refinement not found: %s" % (mtz_map), file=log
            )
            self.mtz_map = None
    ...
    def phenix_reciprocal_space_refinement(self):
        ...
        try:
            outprefix = re.sub(
                r"%s" % (self.maptype),
                "2m%s-DFc_independent_reciprocal_space" % (self.maptype),
                mtz_name,
            )
    ...
    def do_refmac_for_dm(self, pdb_in):
        ...
        mtz_out, pdb_out = refmac_for_dm(
            xyzin=pdb_in,
            hklin=self.mtz_file,
            libins=self.additional.split(),
            f_label=self.get_F_column_labels(self.mtz_file),
        )
        return mtz_out, pdb_out

    def do_density_modification(self, mtz_in, pdb_in, mtz_out, log_file):
        """
        Write script to perform density modification with dm.
        In order to have the correct columns,
        the mtz-file should original from refmac.
        """
        solc = self.get_solvent_content(pdb_in)
        # column labels are those inherited from the extrapolated structure factors
        F_column_labels = self.get_F_column_labels(self.mtz_file)
        dm(
            mtz_in=mtz_in,
            mtz_out=mtz_out,
            solc=solc,
            combine=self.density_modification.combine,
            cycles=self.density_modification.cycles,
            f_label=F_column_labels,
            log_file=log_file,
        )
    ...
    def ccp4_dm(self, pdb_in):
        ...
        """
        mtz_for_dm, pdb_for_dm = self.do_refmac_for_dm(pdb_in)
        ...
        if os.path.isfile(mtz_for_dm) and os.path.isfile(pdb_for_dm):
            ...
            LOG.info(f"Running density modification, see {log_file}. Please wait...")
            self.do_density_modification(mtz_for_dm, pdb_for_dm, mtz_out_dm, log_file)
        return mtz_out_dm
    ...
    def run_refinements(self):
        ...
        print(
            "---Refinement %s with occupancy %.3f---" % (self.maptype, self.occ),
            file=log,
        )
        self.search_mtz_file_reciprocal_space_refinement()
        print(
            "RECIPROCAL SPACE REFINEMENT WITH %s AND %s" % (self.mtz_file, self.pdb_in)
        )
        ...
        if os.path.isfile(pdb_out_rec):
            ...
            print(
                "    pdb-file not found, %s incorrectly returned" % (self.pdb_in),
                file=log,
            )
        ...
        pdb_out_real = self.phenix_real_space_refinement_mtz(
            self.mtz_map, self.pdb_in, maplabels
        )
        ...
        if self.density_modification.density_modification:
            ...
            pdb_out_rec_real = self.phenix_real_space_refinement_ccp4(
            ...
            pdb_out_rec_real = self.phenix_real_space_refinement_mtz(
...
def run(args):
    ...
    print(
        "Xtrapol8 refiner -- Xtrapol8 version %s -- run date: %s" % (__version__, now),
        file=log,
    )
    ...
    if len(args) == 0:
        ...
        raise Usage(
            "phenix.python Xtrapol8_refinements.py + [.phil] + [arguments]\n arguments only overwrite .phil if provided last"
        )
    ...
    Xtrapol8_input_objects = iotbx.phil.process_command_line_with_files(
        args=[params.input.Xtrapol8_out], master_phil=Xtrapol8_master_phil
    )
    ...
    if (
        ...
        == kFextr_calc_map
        == False
        and Xtrapol8_params.f_and_maps.fast_and_furious == False
    ):
        print(
    ...
    if Xtrapol8_params.f_and_maps.fast_and_furious:
        ...
        Xtrapol8_params.f_and_maps.only_no_weight = (
            Xtrapol8_params.f_and_maps.all_maps
        ) = Xtrapol8_params.f_and_maps.only_kweight = (
            Xtrapol8_params.f_and_maps.only_qweight
        ) = False
        ...
        if Xtrapol8_params.map_explorer.occupancy_estimation == "distance_analysis":
            Xtrapol8_params.map_explorer.occupancy_estimation = (
    ...
    for occ in Xtrapol8_params.occupancies.list_occ:
        for maptype in final_maptypes:
            ...
            mtz_out, pdb_rec, pdb_real, pdb_rec_real = Refiner(
                DH.refine_pdb_in,
                maptype,
                outname,
                occ,
                ...
                density_modification=Xtrapol8_params.refinement.phenix_keywords.density_modification,
            ).run_refinements()
    ...
    if params.map_explorer.do_distance_analysis:
        ...
        for mp in reversed_maptypes:
            ...
            alpha, occ = Distance_analysis(
                pdb_list,
                Xtrapol8_params.occupancies.list_occ,
                resids_lst=DH.residue_list,
                use_waters=False,
                outsuffix=mp_type,
                log=log,
            ).extract_alpha()
            ...
            if (
                len(ccp4_list)
                == len(pymol_pdb_list)
                == len(Xtrapol8_params.occupancies.list_occ)
            ):
            ...
            if occ not in Xtrapol8_params.occupancies.list_occ:
                occ = min(
                    Xtrapol8_params.occupancies.list_occ, key=lambda x: abs(x - occ)
                )
                alpha = 1 / occ
            ...
            Difference_distance_analysis(
                DH.X8_pdb_in, pdb_for_ddm, outdir=occ_dir, scale=1.5
            ).ddms()
            print("---------------------------")
            ...
            if len(recref_mtz_lst) != len(Xtrapol8_params.occupancies.list_occ):
                ...
                mtz_rec = recref_mtz_lst[
                    Xtrapol8_params.occupancies.list_occ.index(occ)
                ]
