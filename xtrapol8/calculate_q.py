...
def outlier_rejection_only(f_obs_ref, f_obs_2, log=sys.stdout):
    ...
    if check_common_indices([f_obs_ref, f_obs_2]) == False:
        print(
            "Input files for outlier rejection don't have identical miller indices. Let's try fixing it"
        )
    ...
    for i_bin in f_obs_ref.binner().range_all():
        ...
        print(
            "{:s} {:^10d} {:^6d} outliers for which correction factor c set to 0".format(
                legend, f_obs_ref_bin.size(), sel.count(True)
            ),
            file=log,
        )
        print(
    ...
    SG = re.search(
        r"(.+?)\(No", f_obs_ref.space_group_info().symbol_and_number()
    ).group(1)
    ...
    if check_common_indices([f_obs_ref, c_ms]) == False:
        ...
        if check_common_indices([f_obs_ref, c_ms]) == False:
            print(
                "Outlier rejection array has different indices than data sets. Wrong reflections will be weighted down to zero",
                file=log,
            )
            print(
                "Outlier rejection array has different indices than data sets. Wrong reflections will be weighted down to zero"
...
def calculate_q(f_obs_ref, f_obs_2, log=sys.stdout):
    ...
    print(
        "bin  resolution range  #reflections <deltaFdeconv.est.> <deltaFnon-deconv.est.> <q>  Max.q  Min.q  #outliers  sDeconvolution",
        file=log,
    )
    print(
        "bin  resolution range  #reflections <deltaFdeconv.est.> <deltaFnon-deconv.est.> <q>  Max.q  Min.q  #outliers  sDeconvolution"
    ...
    for i_bin in f_obs_ref.binner().range_all():
        ...
        if deconv >= (nodeconv / 2.0):
            ...
            message = (
                "deconvoluted estimate of differences too small...I have to correct..."
            )
        sigmadsq_lst.append(sigmadsq_bin)
        enumerator_bin = mult_bin.data() * sigmadsq_bin / num_bin.data()
        divisor_bin = (
            (f_obs_ref_bin.sigmas() ** 2) + (f_obs_2_bin.sigmas() ** 2) + enumerator_bin
        )
        ...
        legend = f_obs_ref.binner().bin_legend(i_bin, show_counts=False)
        print(
            "{:s} {:^10d} {:> 10.4f} {:> 20.4f} {:> 12.4f} {:> 6.4f} {:> 6.4f} {:^6d} {:s}".format(
                legend,
                f_obs_ref_bin.size(),
                deconv,
                nodeconv,
    ...
    assert q.size() == f_obs_ref.sigmas().size(), (
        "q has different size than data. Something went wrong during calculation..."
    )
    ...
    if check_common_indices([f_obs_ref, q_ms]) == False:
        ...
        if check_common_indices([f_obs_ref, q_ms]) == False:
            print(
                "Q-weight array has different indices than data sets. Reflections will get wrong weight...",
                file=log,
            )
            print(
                "Q-weight array has different indices than data sets. Reflections will get wrong weight..."
