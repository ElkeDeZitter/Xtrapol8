...
class Difference_analysis:
    def __init__(
        ...
        assert len(fextrfcalc_list) == len(occupancies), (
            "the number of difference maps and occupancies is not equal, this will be nonsense and end with an error somewhere"
        )
    ...
    def check_additional_files(self):
            ...
            for fle in self.additional_files:
                if "cif" in fle:
                    try:
                        ...
                        print(
                            "Unable to read the cif file: %s, this file will be ignored. This might affect some other parts in the analysis."
                            % (fle),
                            file=self.log,
                        )
                    else:
        ...
        SpaceGroup = sgtbx.space_group_info(
            symbol=str(model_in.crystal_symmetry().space_group_info())
        )
        crystal_symmetry = crystal.symmetry(
            unit_cell=model_in.crystal_symmetry().unit_cell(),
    ...
    def do_map_explorer_and_get_data_FoFo(self):
        ...
        Map_explorer_analysis(
            ...
            ligands=self.extract_ligand_codes(),
            log=self.log,
        ).get_ss(self.pdb_in)
        ...
        if self.fofo.endswith("ccp4"):
            self.fofo_data = ccp4_map.map_reader(
                file_name=self.fofo
            ).data.as_numpy_array()
        else:
            self.fofo_data = iotbx.xplor.map.reader(
                file_name=self.fofo
            ).data.as_numpy_array()
    ...
    def get_map_explorer_results_FextrFcalc(self, mask=None):
        ...
        for i, fextrfcalc in enumerate(self.fextrfcalc_list):
            ...
            if fextrfcalc.endswith("ccp4"):
                ...
                data = iotbx.xplor.map.reader(
                    file_name=fextrfcalc
                ).data.as_numpy_array()
            pos = 0
            neg = 0
            ...
            try:
                ...
                print(
                    "Mask not found. Run get_map_explorer_results_FoFo first and make sure it ran correctly."
                )
                return
    ...
    def run_difference_map_analysis(self):
        ...
        _, _, _, _ = plotalpha(
            self.occupancies,
            self.map_exp_files[1:],
            self.map_exp_files[0],
            self.prefix,
            log=self.log,
        ).estimate_alpha()
...
class Filefinder:
    ...
    def find_fextfc(self):
        ...
        for occ in self.X8_list_occ:
            f = "{:s}/{:s}occupancy_{:.3f}/{:s}_occ{:.3f}_{:s}".format(
...
def main():
    ...
    fofo_map = Filefinder(
        ...
        X8_fofo_type=Xtrapol8_params.f_and_maps.fofo_type,
    ).find_fofo()
    ...
    fextrfcalcs = Filefinder(
        ...
        diffmap_f_extrapolated_and_maps=params.input.f_extrapolated_and_maps,
    ).find_fextfc()
    ...
    if len(fextrfcalcs) != len(occupancies):
        print(
            "Number of occupancies and mFextr-DFcalc maps is not equal. Please provide a single occupancy for each map."
        )
        sys.exit(1)
