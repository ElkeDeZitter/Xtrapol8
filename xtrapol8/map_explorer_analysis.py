...
class Map_explorer_analysis(object):
    ...
    def __init__(self, peakintegration_file=None, ligands=[], log=sys.stdout):
        self.log = log
        if peakintegration_file != None:
            ...
            self.peaklines = [
                ...
                if len(line.split()) > 1 and ("Resn" not in line)
            ]
        self.check_peaks()
    ...
    def check_peaks(self):
        try:
            ...
            print(
                "No peaks identified. Choose better map_explorer parameters.",
                file=self.log,
            )
    ...
    def get_sorted_peaklines_ligands(self):
        ...
        peaklines_ligands = [
            lne for lne in self.peaklines for ligand in self.ligands if (ligand) in lne
        ]
        cnp = [
            [
                "-".join([line.split()[2], line.split()[0], line.split()[1]]),
                line.split()[-2],
            ]
    ...
    def peaksum_per_residue(self, peaklist):
        ...
        while x < len(peaklist) - 1:
            try:
                if ",".join(str(peaklist[x][0:-1])) == ",".join(
                    str(peaklist[x + 1][0:-1])
                ):
    ...
    def get_ss(self, pdb_file):
                ...
                for res_group in chain.residue_groups():
                    for atom_group in res_group.atom_groups():
                        ...
                        for lig in self.ligands:
                            if (
                                atom_group.resname == lig
                                or atom_group.atoms_size() == 0
                            ):
        ...
        for helix in results.helices:
            helix_range = (
                helix.start_chain_id,
                helix.get_start_resseq_as_int(),
                helix.get_end_resseq_as_int(),
            )
        ...
        for sheet in results.sheets:
            for strand in sheet.strands:
                strand_range = (
                    strand.start_chain_id,
                    strand.get_start_resseq_as_int(),
                    strand.get_end_resseq_as_int(),
                )
        ...
        if len(self.ligands) > 0:
            if self.peaklines != None:
                pos_peaklist_ligands, neg_peaklist_ligands = (
                    self.get_sorted_peaklines_ligands()
                )
                ligands = list(
                    set(
                        [
                            peaklist[x][0]
                            for peaklist in [pos_peaklist_ligands, neg_peaklist_ligands]
        ...
        chains = [
            chain.id for chain in hier.chains()
        ]  # if chain.is_protein()] deleted for DNA
        ...
        mn, mx = self.get_plot_limits(
            pos_peaklist, neg_peaklist, pos_peaklist_ligands, neg_peaklist_ligands
        )
        if mn == mx:
            mx += 0.001
            mn -= 0.001
        ...
        for chain in hier.chains():
            ...
            if ID not in chain_dict.keys():
                ...
                helix_ranges = [
                    range(lne[1], lne[2] + 1, 1) for lne in helix_lst if ID in lne[0]
                ]
                strand_ranges = [
                    range(lne[1], lne[2] + 1, 1) for lne in strand_lst if ID in lne[0]
                ]
                try:
                    # get number of amino acids, won't work if multiple conformers
                    AA = flex.double(
                        [
                            chain.residues()[0].resseq_as_int(),
                            chain.residues()[-1].resseq_as_int(),
                        ]
                    )
                    ...
                    AA = flex.double(
                        [
                            chain.conformers()[0].residues()[0].resseq_as_int(),
                ...
                strand_ranges_flat = [
                    val for sublist in strand_ranges for val in sublist
                ]
                ...
                axs[chain_dict[ID]].bar(
                    pos_peak_resids, pos_peaks, color="green", width=1.0
                )
                axs[chain_dict[ID]].bar(
                    neg_peak_resids, neg_peaks, color="red", width=1.0
                )

                axs[chain_dict[ID]].set_ylim(mn, mx)
                axs[chain_dict[ID]].set_title(
