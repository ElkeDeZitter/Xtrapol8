...
def calculate_k(f_obs_ref, f_obs_2, kweight_scale=0.05, log=sys.stdout):
    ...
    if check_common_indices([f_obs_ref, f_obs_2]) == False:
        print(
            "Input files for k-weighthing don't have identical miller indices. Let's fix it"
        )
    ...
    if check_common_indices([f_obs_ref, k_ms]) == False:
        ...
        if check_common_indices([f_obs_ref, k_ms]) == False:
            print(
                "k-weight array has different indices than data sets. k-weighted maps will be nonsense",
                file=log,
            )
            print(
                "k-weight array has different indices than data sets. k-weighted maps will be nonsense"
    ...
    ax1.plot(
        bin_res_cent_lst[1:], k_av_lst[1:], marker=".", label="Average k", color="red"
    )
