...
eof"
        % (mtz_in, mtz_out, b_scaling, low_res, high_res)
    )
    i.close()
...
def run_scaleit(
    f_obs_ref, f_obs_2, b_scaling, low_res=None, high_res=None, log=sys.stdout
):
    ...
    mtz_fromscaleit, script_scaleit = write_scaleit_input(
        mtz_forscaleit, b_scaling, low_res, high_res
    )
    os.system("chmod +x %s" % (script_scaleit))
    print("Running scaleit, see %s" % (script_scaleit), file=log)
    os.system("./%s" % (script_scaleit))

    reflections = any_file(
        mtz_fromscaleit, force_type="hkl", raise_sorry_if_errors=True
    )
