...
class Difference_distance_analysis(object):
    ...
    def __init__(self, pdb1, pdb2, ligands=[], outdir=None, scale=1.5, log=sys.stdout):
        ...
        print(
            "number of protein atoms for ddm calculation: %d"
            % (self.hier1.atoms_size()),
            file=self.log,
        )
    ...
    def get_last_residue_number(self, pdb_hierarchy, ID):
        """
        Function to get the last residue number of a certain chain
        """
        last = 0
    ...
    def ddm_residue(self, ddm, df):
        ...
        # set all values from the missing residues to 0
        missing = df.loc[df["present"] == 0, "resseq"]
        to_zero = [i for i, val in enumerate(seq_info_unique) if val in missing]
        if len(to_zero) >= 1:
            for i in to_zero:
                ddm_residue[i, :] = 0
        ...
        for chain in self.hier1.chains():
            if chain.is_protein():
                ...
                if ID not in chain_dict.keys():
                    ...
                    if self.scale == None:
                        self.scale = max(
                            np.abs(np.min(ddm_residue)), np.abs(np.max(ddm_residue))
                        )
                    # else:
                    # scale = self.scale

                    # if (n_rows > 1 and n_cols > 1):
                    img = axs[chain_dict[ID]].imshow(
                        FINAL2, cmap=rvb, vmin=-self.scale, vmax=self.scale
                    )
...
def main():
    parser = argparse.ArgumentParser(
        "Calculate the difference distance matrix between two pdb files."
    )
    parser.add_argument(
        "-r", "--pdb_ref", type=str, default=None, help="Reference pdb file."
    )
    parser.add_argument(
        "-o",
        "--pdb_other",
        type=str,
        default=None,
        help="Other pdb file to be compared with the reference pdb file.",
    )
    parser.add_argument(
        "-l",
        "--ligand",
        type=str,
        action="append",
        help="Ligand 3-letter code. Add this argument for each ligand.",
    )
    parser.add_argument(
        "-d",
        "--outdir",
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Scale for ddm. Default=None, the scale will be determined on the ddm of the first chain.",
    )
    ...
    ddm_out = Difference_distance_analysis(
        pdb_ref, pdb_other, ligands=ligands, outdir=outdir, scale=scale
    ).ddms()
