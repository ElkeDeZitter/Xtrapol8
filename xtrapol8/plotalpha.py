...
class plotalpha(object):
    ...
    def get_final_list(self, map_expl_fle):
        ...
        lines = np.array(
            lines
        )  # lines contains the usefull lines from the peakintegration file
        finallst = []  # finallist will contains all the lines from "lines" for the residues in residuelist
        if (
            self.resids_lst == None
        ):  # If no residlist provided, then all residues will be used
            finallst = lines
        else:
            for resid in self.residlst:
                test = [
                    line
                    for line in lines
                    if ",".join(resid.split()) in ",".join(line.split())
                ]  # make sure all residues in residue list are in peakintegration file. This avoids addition of residues for which no peaks are found because this would mess up the analysis
                for lne in test:
                    if len(lne) > 0:
    ...
    def get_integrated_values(self, map_expl_fle):
        ...
        for line in finallst:
            tmp += np.abs(float(line.split()[-2])) / float(
                line.split()[-1]
            )  # sum all the peak-values to tmp for residues in finallist
        for line in lines:
            tmp2 += np.abs(float(line.split()[-2])) / float(
                line.split()[-1]
            )  # sum all the peak-values to tmp2
            intnoise += np.abs(float(line.split()[-2])) / float(
                line.split()[-1]
            )  # all peaks will be used to calculate the noise
        ...
        for i, expl_fle in enumerate(self.map_expl_files):
            ...
            if i == 0:  # i==0 means that 'expl_fle' is the peaklist from the Fo-Fo map
                try:
                    ...
                    self.exp1[0] = (
                        tmp  #'experimental' peak-height = sum(all selected)/sqrt(all peaks)
                    )
                    self.exp2[0] = (
                        tmp2  #'experimental' peak-height = sum(all)/sqrt(all peaks)
                    )
                ...
                try:
                    noise = np.sqrt(intnoise)
                    self.int1[i - 1] = (
                        tmp  # / noise #'integrated' peak-height = sum(all selected)/sqrt(all peaks)
                    )
                    self.int2[i - 1] = (
                        tmp2  # / noise #''integrated' peak-height = sum(all)/sqrt(all peaks)
                    )
                except:  # exception in which case? ZeroDivisionError?
                    self.int1[i - 1] = tmp
                    self.int2[i - 1] = tmp2
                print(
                    "occ: %.3f alpha:%.3f integrated peak area selected: %.3f integrated peak area all: %.3f"
                    % (
    ...
    def alphadetermination(self):
        ...
        try:
            ...
            self.results = self.normalize_array(
                self.sum
            )  # self.normalize_array(results) #normalization
    ...
    def alphadetermination_special(self):
        ...
        gooda = np.where(
            self.results == np.nanmax(self.results)
    ...
    def plot_alpha_determination(self, outname):
        ...
        out = open(
            "alpha_occupancy_determination_%s.pickle" % (self.outsuffix), "wb"
