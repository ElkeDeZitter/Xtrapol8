...
def plot_negativereflections(prefix):
    ...
    ax1.plot(
        bin_res_cent_lst[:], neg_lst[:], marker=".", label="# Neg. ESFAs", color="red"
    )
    ax1.set_xlim(np.max(bin_res_cent_lst[1:]), np.min(bin_res_cent_lst[1:]))
    ...
    ax3.plot(
        bin_res_cent_lst[:], comp_lst[:], marker=".", label="Completeness", color="red"
    )
    ax3.plot(
        bin_res_cent_lst[:],
        comp_true_lst[:],
...
def plot_q_estimation(pickle_file="q_estimation.pickle"):
    ...
    ax1.plot(
        bin_res_cent_lst[:], q_av_lst[:], marker=".", label="Average q", color="red"
    )
    ax1.tick_params(axis="y")
    ax1.set_xlim(np.max(bin_res_cent_lst[1:]), np.min(bin_res_cent_lst[1:]))
    ax1.set_ylim(0, 1)
...
def plot_alpha_occupancy_determination(suffix):
    ...
    axes[0, 0].plot(
        alphas, neg_features, "s", markersize=5, color="red", label="Negative features"
    )
    ...
    if alphafound:
        ...
        axes[1, 1].vlines(
            occ_CC, ymin=0, ymax=np.max(pearsonCC), color="magenta", linestyles="dashed"
        )
        ...
        axes[0, 1].set_title(
            "Occupancy determination IMPOSSIBLE", fontsize="medium", fontweight="bold"
        )
        axes[0, 1].text(
            np.min(occupancies),
            0.5,
...
def plot_ddm(pickle_file="ddm.pickle", scale=1.5):
    ...
    for chain in alldata:
        ...
        plt.setp(
            axs[row, col].xaxis.get_majorticklabels(), rotation=90, fontsize="small"
        )
