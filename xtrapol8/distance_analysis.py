    ...
    if np.all(sel == 0):
        ...
        try:
            ...
            popt, _ = curve_fit(
                ...
                bounds=((Lmin, kmin, 1), (Lmax, kmax, x0max)),
            )
        except ValueError:
...
class Distance_analysis:
    ...
    def __init__(
        ...
        if plateau_fraction < 0.0:
            print(
                "Distance analysis plateau fraction should be set between 0.0 and 1.0"
            )
        ...
        if self.resids_lst == None:
            print(
                "Using all atoms for distance analysis is computationally heavy and can take a while."
            )
            self.get_residlist_from_all_atoms(self.pdblst[0])
        print(
            "Distance analysis based on %d residues" % (self.residlst.shape[0]),
            file=self.log,
        )
        ...
        for chain in hier.chains():
            ...
            for res_group in chain.residue_groups():
                ...
                for atom_group in res_group.atom_groups():
                    ...
                    if self.use_waters == False and resn == "HOH":
                        continue
    ...
    def get_coord_from_parser_selected(self, pdb_file):
        ...
        for chain_to_check in chains_to_check:
            resids = [
                (resid.split()[0], resid.split()[1])
                for resid in self.residlst
                if resid.split()[2] == chain_to_check
            ]
            for chain in hier.chains():
                if chain.id == chain_to_check:
                    for res_group in chain.residue_groups():
                        for atom_group in res_group.atom_groups():
                            if (
                                atom_group.resname,
                                res_group.resseq.lstrip().rstrip(),
                            ) in resids:
    ...
    def select_common_coords(self, coord, info):
        ...
        _, _, indices_retain = np.intersect1d(
            self.common_info, info_ar, return_indices=True
        )
    ...
    def get_all_distances(self):
        ...
        assert (
            self.info.shape[0]
            == self.alldistances.shape[1]
            == self.alldistances.shape[2]
            == self.alldifferences.shape[1]
            == self.alldifferences.shape[2]
        )

        try:
            print("max sampled distance: %s" % (np.max(alldistances)), file=self.log)
            print(
                "properties of differences (max, mean, median): %s /// %s /// %s "
                % (
                    np.max(alldifferences),
                    np.mean(alldifferences),
                    np.median(alldifferences),
                ),
                file=self.log,
            )
    ...
    def plot_distance_differences_and_get_possible_b(
        self, maxdist=10.0, mindist=2.0, maxdiff=10, mindiff=0.25
    ):
        info_for_print = [
            [
                i[0],
                i[2],
                i[1].lstrip().rstrip(),
                "(%s%s)" % (i[4].lstrip().rstrip(), i[3].lstrip().rstrip()),
            ]
            for i in self.info
        ]
        ...
        assert (
            fitting_matrix.shape[1]
            == fitting_matrix.shape[2]
            == self.alldifferences.shape[1]
        )
        ...
        res_matrix = np.subtract(
            self.alldifferences,
            np.apply_along_axis(sigmoid_fit_2, axis=0, arr=fitting_matrix, x=x),
        )
        assert res_matrix.shape == self.alldifferences.shape

        # calculate sum of squares of the residuals
        sumofsquares_res_matrix = np.apply_along_axis(
            sumofsquares, axis=0, arr=res_matrix
        )
        assert sumofsquares_res_matrix.shape == self.alldifferences.shape[1:]

        # calculate total sum of squares of the differences
        totalsumsquares_matrix = np.apply_along_axis(
            totalsumsquares, axis=0, arr=self.alldifferences
        )
        assert totalsumsquares_matrix.shape == self.alldifferences.shape[1:]

        # calculate chi square
        chisq_matrix = np.nansum(
            np.divide(res_matrix, np.std(self.alldifferences, axis=0)) ** 2, axis=0
        )
        assert chisq_matrix.shape == self.alldifferences.shape[1:]

        # calculate r square
        r_squared_matrix = np.subtract(
            1, np.divide(sumofsquares_res_matrix, totalsumsquares_matrix)
        )
        ...
        if np.where(np.any(toplot_matrix == 1, axis=1))[0].shape[0] < 400:
            print(
                "Less than 400 atoms involved in distance calculation. Let's plot all distances:",
                file=self.log,
            )
            print(
                "     Distance                                  R2   chi2   occupancy",
                file=self.log,
            )
            print(
                "Less than 400 atoms involved in distance calculation. Let's plot all distances:"
                ...
                for j in toplot2:
                    ...
                    print(
                        "{:^40s} {:>10.2f} {:>5.2f} {:>5.2f}".format(
                            title,
                            r_squared_matrix[n, j],
                            chisq_matrix[n, j],
                            1
                            / (
                                (val / fitting_matrix[1, n, j])
                                + fitting_matrix[2, n, j]
                            ),
                        ),
                        file=self.log,
                    )
                    ...
                    ax0.plot(
                        x,
                        self.alldifferences[:, n, j],
                        color="%s" % (colorlib[a]),
                        linestyle=":",
                        linewidth=0.30,
                        label="Individual distance",
                    )
                    ax0.plot(
                        x,
                        np.abs(sigmoid_fit_2(fitting_matrix[:, n, j], x)),
                        "r--",
                        linewidth=0.20,
        ...
        ax0.plot(
            xx,
            sigmoid_fit(
                xx, np.mean(possible_amp), np.mean(possible_b), np.mean(possible_x0)
            ),
            "b--",
            label="Average of all fits",
        )
        ...
        ax0.legend(
            ...
            labels,
            loc="lower right",
            bbox_to_anchor=(0.85, -0.05, 0.45, 0.5),
            fontsize="xx-small",
            framealpha=0.5,
        )
        ...
        # Set figure title
        fig.suptitle(
            "Distance analysis between reference model and real-space-refined models",
            fontsize="medium",
            fontweight="bold",
        )
        # Set figure layour
        ...
        # plt.show()
        plt.savefig(
            "Distance_difference_plot_%s.pdf" % (self.outsuffix),
            dpi=300,
            transparent=True,
        )
    ...
    def extract_alpha(self):
        ...
        if np.all(poptave == np.array([0.0, 0.0, 0.0])):
        ...
        if possible_b.shape[0] > 0:
            ...
            try:
                ...
                print(
                    "Occupancy based on average of fit results for %d distances: %.3f +/- %.3f"
                    % (np.sum(counts), occ_mean, occ_stdev),
                    file=self.log,
                )
                print(
                    "  Occupancy intervals : %.3f  > %.3f >  %.3f "
                    % (occ_interval_max, occ_mean, occ_interval_min),
                    file=self.log,
                )
                print("Other:", file=self.log)
                print(
                    "  Occupancy based on histogram of fit results for %d distances: %.3f"
                    % (np.sum(counts), occ_histogram),
                    file=self.log,
                )
                print(
                    "  Occupancy based on most occuring value in histogram: %.3f"
                    % (occ_mode),
                    file=self.log,
                )
                print(
                    "  Occupancy based on fit of average distance: %.3f"
                    % (occ_av_distance),
                    file=self.log,
                )
            ...
            print(
                "No distance differences within allowed range, alpha and occupancy cannot be estimated.",
                file=self.log,
            )
            print(
                "No distance differences within allowed range, alpha and occupancy cannot be estimated."
...
class Filefinder:
    ...
    def find_pdbs(self):
        ...
        for occ in self.X8_list_occ:
            d = "{:s}/{:s}occupancy_{:.3f}/{:s}_occ{:.3f}_{:s}".format(
                self.X8_outdir, first_part, occ, self.X8_outname, occ, last_part
            )
...
def main():
    ...
    input_objects = iotbx.phil.process_command_line_with_files(
        args=sys.argv[1:], master_phil=master_phil
    )
    ...
    pdbs = Filefinder(
        X8_outdir=Xtrapol8_params.output.outdir,
        ...
        distance_f_extrapolated_and_maps=params.input.f_extrapolated_and_maps,
    ).find_pdbs()

    pdblst = [model_pdb] + pdbs
    occupancies = Xtrapol8_params.occupancies.list_occ

    assert len(pdblst) == len(occupancies) + 1, (
        "Please provide a model PDB and an occupancy value for each pdb in the PDB list"
    )
