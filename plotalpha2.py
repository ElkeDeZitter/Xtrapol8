...
class plotalpha(object):
    ...
    """
    def __init__(self, occupancies, integrated_values, pearsonCC, reference, outsuffix='qFextr', log=sys.stdout):

        self.occupancies = np.array(occupancies)
        self.alphas = np.reciprocal(occupancies)
        self.integrated_values = integrated_values
        self.reference = reference
        self.pearsonCC = pearsonCC
    ...
    def alphadetermination(self):
        ...
        try:
            ...
            self.results = self.normalize_array(self.integrated_values[:,2])#self.normalize_array(results) #normalization
        except ZeroDivisionError:
            print("no peaks found in at least one of the maps")
            self.results = np.zeros_like(self.integrated_values[:,2])
    ...
    def plot_alpha_determination(self, outname):
        ...
        stats = [self.alphas,self.occupancies, self.integrated_values[:,0], self.integrated_values[:,1], self.results]
        stats.append(True)
        ...
        axes[0, 0].plot(self.alphas, self.integrated_values[:,2] / self.reference[2], '^', color="k", label='All features')
        axes[0, 0].vlines(self.alpha, ymin=0, ymax=np.max(self.integrated_values[:,2] / self.reference[2]))
        ...
        axes[0, 1].plot(self.occupancies, self.integrated_values[:, 2] / self.reference[2], '^', color="k", label='All features')
        axes[0, 1].vlines(self.occ, ymin=0, ymax=np.max(self.integrated_values[:, 2] / self.reference[2]))
    ...
    def estimate_alpha(self):
        ...
        for i in range(self.alphas.shape[0]):
            ...
            print("{:>5.3f} {:>15.3f} {:>15.3f} {:>10.3f} {:>20.3f}".format(self.occupancies[i],self.alphas[i], self.integrated_values[i, 2], self.integrated_values[i, 2] / float(self.reference[2]), self.pearsonCC[i]), file=self.log)

            print("{:>5.3f} {:>15.3f} {:>15.3f} {:>10.3f} {:>20.3f}".format(self.occupancies[i],self.alphas[i], self.integrated_values[i, 2], self.integrated_values[i, 2] / float(self.reference[2]), self.pearsonCC[i]))
